...
func _input(event: InputEvent) -> void: #input(viewport: Node, event: InputEvent, shape_idx: int):
	...
	if event is InputEventMouseMotion:
		...
		var territory_id = $"../Sprites/ColorID".get_id_by_position( mouse_pos )
		...
		if territory_id != hover_territory_id:
			...
			$"../Sprites/ColorID".set_sprite_to_territory( hover_territory_id, $HoverVisualizer )
